...
class FasterRCNN(nn.Module):
    ...
    @torch.no_grad()
    def inference_test_step(self, batch):
        # Assume batch['image'] is a list of tensors
        images = [img.cuda() for img in batch['image']]
        ...
        for b_idx in range(len(images)):
            ...
            boxes = out[b_idx]['boxes']
            labels = out[b_idx]['labels']
            labels[labels == 2] = 1 # merge the weeds and plants into a single class 1
            ...
            surviving_dict = {}
            if high_scores.sum():
                surviving_dict['boxes'] = refined_boxes[high_scores].cuda()
                surviving_dict['labels'] = refined_labels[high_scores].cuda()
                surviving_dict['scores'] = refined_scores[high_scores].cuda()
            else:
                surviving_dict['boxes'] = torch.empty((0, 4)).cuda()
                surviving_dict['labels'] = torch.empty(0).cuda()
                surviving_dict['scores'] = torch.empty(0).cuda()
